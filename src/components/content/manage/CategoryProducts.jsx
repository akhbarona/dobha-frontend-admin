...
const CategoryProducts = () => {
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      await AuthService.addCategoryProduct(namaCategory).then(
        (res) => {
          console.log(res);
          Swal.fire({
            icon: 'success',
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const onDelete = async () => {
          await AuthService.deleteCategoryProduct(id)
            .then((res) => {
    ...
    },
    {
      ...
      Cell: (row) => (
        <div className="d-flex d-flex justify-content-around">
          <button className="btn w-45 btn-warning" onClick={() => navigate(`/create-category-products/${row.row.original.id}`)}>
            Update
          </button>
    ...
    <Container fluid>
      <Row>
        <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 className="h2">Manage Category Product</h1>
          <div className="btn-toolbar mb-2 mb-md-0"></div>
        </div>
        <div className="pt-2" style={{ fontWeight: '500' }}>
          <Form onSubmit={handleAddCategory}>
            <Form.Group className="mb-3" controlId="formBasicCategory">
              <Form.Label className="h5">Category Product</Form.Label>
